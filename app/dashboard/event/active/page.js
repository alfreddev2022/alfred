...
const Page = () => {
  ...
  const products = events.filter(e=>moment(currentDate).isBefore(e.expired));
  ...
  return (
    <div className="p-4 pl-[12rem] pt-[7rem] ">
      <div className="container mx-auto">
       ...
       { currentProducts.length >= 9 && <div className="flex justify-center mt-4">
          {products && [...Array(Math.ceil( products.length / productsPerPage)).keys()].map(
            (number) => (
              <button
                key={number + 1}
